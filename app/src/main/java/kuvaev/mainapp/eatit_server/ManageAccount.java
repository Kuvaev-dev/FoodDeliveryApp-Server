...
import kuvaev.mainapp.eatit_server.ViewHolder.UserViewHolder;
...
public class ManageAccount extends AppCompatActivity {
    ...
    private void loadAccount() {
        ...
        adapter = new FirebaseRecyclerAdapter<User, UserViewHolder>(options) {
            ...
            protected void onBindViewHolder(@NonNull UserViewHolder viewHolder, final int position, @NonNull final User model) {
                ...
                // new event
                viewHolder.btnEditAccount.setOnClickListener(v -> showEditAccountDialog(adapter.getRef(position).getKey(), model));
                viewHolder.btnDeleteAccount.setOnClickListener(v -> showDeleteAccountDialog(adapter.getRef(position).getKey()));
            }
    ...
    private void showCreateAccountDialog() {
        AlertDialog.Builder alertDialog = new AlertDialog.Builder(ManageAccount.this, androidx.appcompat.R.style.Theme_AppCompat_DayNight_Dialog_Alert);
        ...
        alertDialog.setPositiveButton("CREATE", (dialog, which) -> {
            ...
            if (TextUtils.isEmpty(account_phone.getText())) {
            ...
            } else if (Objects.requireNonNull(account_phone.getText()).length() < 11    ){
                ...
                user.setName(Objects.requireNonNull(account_name.getText()).toString());
                user.setPassword(Objects.requireNonNull(account_password.getText()).toString());
                user.setIsstaff("true");
                user.setIsadmin("false");

                users.child(account_phone.getText().toString())
                        .setValue(user)
                        .addOnSuccessListener(aVoid -> Toast.makeText(ManageAccount.this, "Staff Created Successfully!", Toast.LENGTH_SHORT).show())
                        .addOnFailureListener(e -> Toast.makeText(ManageAccount.this, "Failed to Create Account!", Toast.LENGTH_SHORT).show());
            }
